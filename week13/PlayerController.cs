...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D whatDidIHit)
    {
        if (whatDidIHit.tag == "Powerup")
        {
            ...
            switch (whichPowerup)
            {
                ...
                case 2: // double weapon
                    weaponType = 2;
                    StartCoroutine(WeaponPowerDown());
                    gameManager.ManagePowerupText(2);
                    break;

                case 3: // triple weapon
                    weaponType = 3;
                    StartCoroutine(WeaponPowerDown());
                    gameManager.ManagePowerupText(3);
                    break;

                case 4: // shield- protects player for one hit
                    if (!hasShield)
                    {
                        hasShield = true;
    ...
    void Movement()
    {
        ...
        if (transform.position.x <= -horizontalScreenSize + horizontalOffset ||
            transform.position.x >= horizontalScreenSize - horizontalOffset)
        {
            transform.position = new Vector3(transform.position.x * -1, transform.position.y, 0);
        }

        // movement limits
        float minY = -verticalScreenSize + 2.5f;   // bottom line limit
        float maxY = 0f;                           // halfway point
