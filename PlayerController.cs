...
public class PlayerController : MonoBehaviour
{
    ...
    private bool hasShield;               // Tracks shield state
    private GameObject activeShield;      // Instance of active shield
    ...
    void Start()
    {
        ...
        shieldPrefab.SetActive(false); // keep shield hidden at start
    ...
    public void LoseALife()
    {
        if (hasShield)
        {
            ...
            gameManager.PlaySound(2); // power-down sound
            return;
        }
    ...
    {
        yield return new WaitForSeconds(5f); // shield duration
        if (hasShield)
        {
            hasShield = false;
            activeShield.SetActive(false);
